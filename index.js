...
const baseUrl = `http://10.210.44.8:4444/wd/hub/`;
let sessionId = 'null';
let sessionUrl = `null`;
...
class Action {
	constructor(url, method, json, desc, expect, time = 60000) {
		this.body = {
			uri: `${url}`,
			method: method,
			json: json,
...
module.exports = {
	newChromeSession: function() {
		const chromeOptions = {
			'desiredCapabilities': {
				...
				'browserName': 'chrome',
				'chromeOptions': {
					//'binary': `C:\\Users\\ContainerAdministrator\\appdata\\Local\\Google\\Chrome SxS\\Application\\chrome.exe`, // canary location
					//'binary': `C:\\Program Files (x86)\\google\\chrome\\Application\\chrome.exe`, //regular location
	...
	elements: function(selector, index, timeout = 60000) {
		q.push(new Action(`elements`, 'POST', { using: 'css selector', value: selector }, `Get single element locator from a list`, index, true, timeout), (result) => {
